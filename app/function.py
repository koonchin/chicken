...
def write_pdf(id):
    ...
    template_path = f'{settings.MEDIA_ROOT}/pedigree_templates_2.png'
    task = f"""
    select 
    chicken.path_img, 
    ...
    if mainpath:
        mainpath = os.path.join(settings.BASE_DIR,mainpath)
        main_chicken = Image.open(mainpath)
        main_chicken = main_chicken.resize((1500,2130), Image.LANCZOS)

    # M - F
    if Mpath:
        Mpath = os.path.join(settings.BASE_DIR,Mpath)
    ...
    bias = 30
    bias_sm = 50
    title_font = ImageFont.truetype(f'{settings.BASE_DIR}/static/font/ChakraPetch-Light.ttf', 100)
    small_font = ImageFont.truetype(f'{settings.BASE_DIR}/static/font/ChakraPetch-Light.ttf', 70)
...
def generate_qr(url,id):
    img = qrcode.make(f'https://170-187-198-35.ip.linodeusercontent.com/share/{id}')
    img.save(f'{settings.MEDIA_ROOT}/{id}.png')
    return f'{settings.MEDIA_ROOT}/{id}.png'

def upload_img(imagepath):
    head, imagename = os.path.split(imagepath)
    with open(imagepath,'rb') as f:
        image = base64.b64encode(f.read())
    params = {
    'key': '617a5159bb15cb66953a95b1e465b76d',
    }
...
def upload_image():
    for i in os.listdir('media'):
        year = os.path.join('media',i)
        for month in os.listdir(year):
            months = os.path.join(year,month)
            for day in os.listdir(months):
                days = os.path.join(months,day)
                for file in os.listdir(days):
                    try:
                        upload_img(os.path.join(days,file))
                    except Exception as e:
                        print(e,file)
